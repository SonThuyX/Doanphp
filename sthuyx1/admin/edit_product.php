...
$page_title = "Sửa Sản phẩm - Trang quản trị";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE products SET name = ?, price = ?, image = ?, category_id = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sisii", $name, $price, $image, $category_id, $id);

    if ($stmt->execute()) {
        $_SESSION['message'] = "Cập nhật sản phẩm thành công!";
    } else {
        $_SESSION['message'] = "Cập nhật sản phẩm thất bại: " . $conn->error;
    }
    $stmt->close();
    header("Location: index.php");
    exit();
...
include 'html/edit_product_view.php';
?>